...
for (LANG in LANGS) {
  ...
  for (TERRITORY in TERRITORIES) {
    ...
    ### 3.3 Plot ----
    # ----------------------------------------------------------------------- - - -
    # Two vertically stacked panels (facets), same X (2001–2024), free Y per panel.
    df_fo <- df_plot %>% dplyr::filter(series == "fo")
    df_nf <- df_plot %>% dplyr::filter(series == "nf")

    # EN: multilingual pieces
    territory_title <- TERRITORY_LABELS[[TERRITORY]]
    subtitle_fo     <- label("panel_fo")
    subtitle_nf     <- label("panel_nf")
    legend_text_fo  <- label("legend_fo")
    legend_text_nf  <- label("legend_nf")

    # EN: build FO plot (top) — red, with its own legend and pretty Y axis
    p_fo <- ggplot(df_fo, aes(x = year, y = area_ha, color = series, linetype = series)) +
      geom_line(linewidth = 1.2, lineend = "round", na.rm = TRUE) +
      geom_point(size = 3.5, stroke = 0, na.rm = TRUE) +
      scale_color_manual(values = c(fo = source_line_colors[["fo"]]),
                        breaks = "fo", labels = c(fo = legend_text_fo)) +
      scale_linetype_manual(values = c(fo = source_line_types[["fo"]]),
                            breaks = "fo", labels = c(fo = legend_text_fo)) +
      axis_x_years_all(BURN_YEAR_MIN, BURN_YEAR_MAX) +
      axis_y_ha_auto(max(df_fo$area_ha, na.rm = TRUE)) +   # EN: no scientific notation
      labs(
        title     = label("title_burned_in", territory = territory_title),  # EN: global title on the top plot
        subtitle  = subtitle_fo,                                            # EN: bigger panel header
        x         = label("x_year"),
        y         = label("y_area_ha"),
        caption   = NULL
      ) +
      ...
      theme(
        legend.position = "top",
        plot.margin     = margin(6, 12, 4, 12)  # EN: small bottom margin to create separation
      )

    # EN: build NF plot (bottom) — orange, with its own legend and pretty Y axis
    p_nf <- ggplot(df_nf, aes(x = year, y = area_ha, color = series, linetype = series)) +
      geom_line(linewidth = 1.2, lineend = "round", na.rm = TRUE) +
      geom_point(size = 3.5, stroke = 0, na.rm = TRUE) +
      scale_color_manual(values = c(nf = source_line_colors[["nf"]]),
                        breaks = "nf", labels = c(nf = legend_text_nf)) +
      scale_linetype_manual(values = c(nf = source_line_types[["nf"]]),
                            breaks = "nf", labels = c(nf = legend_text_nf)) +
      axis_x_years_all(BURN_YEAR_MIN, BURN_YEAR_MAX) +
      axis_y_ha_auto(max(df_nf$area_ha, na.rm = TRUE)) +   # EN: no scientific notation
      labs(
        title     = NULL,                # EN: no repeated main title here
        subtitle  = subtitle_nf,         # EN: bigger panel header
        x         = label("x_year"),
        y         = label("y_area_ha"),
        caption   = caption_burned(TERRITORY, LANG)  # EN: keep caption only once (bottom)
      ) +
      theme_time_series() +
      theme(
        legend.position = "top",
        plot.margin     = margin(10, 12, 10, 12)     # EN: extra margins for separation and caption
      )

    # EN: stack the two plots with a small vertical gap (rel_heights can emphasize top vs bottom if desired)
    p <- cowplot::plot_grid(
      p_fo, p_nf,
      ncol = 1, align = "v", rel_heights = c(1, 1), labels = NULL
    )

    print(p)
    message(glue("✓ Plot (two stacked panels with own legends) generated for {TERRITORY}"))
