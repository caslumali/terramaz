...
MB_CLASS_I18N <- list(
  fr = c(
    "Forest"                        = "Forêt",
    "Non-forest natural formation"  = "Formation naturelle non forestière",
    "Water"                         = "Eau",
    "Pasture"                       = "Pâturage",
    ...
    "Other anthropic"               = "Autres anthropiques", # fr
    "Other"                         = "Autres"
  ),
  es = c(
    "Forest"                        = "Bosque",
    "Non-forest natural formation"  = "Formación natural no forestal",
    "Water"                         = "Agua",
    "Pasture"                       = "Pasto",
    "Agriculture"                   = "Agricultura",
    "Silviculture"                  = "Silvicultura",
    "Oil palm"                      = "Palma aceitera",
    "Mosaic of uses"                = "Mosaico de usos",
    "Urban"                         = "Urbano",
    "Mining"                        = "Minería",
    "Other anthropic"               = "Otras antrópicas"  ,
    "Other"                         = "Otros"
  ),
  pt = c(
    "Forest"                        = "Floresta",
    "Non-forest natural formation"  = "Formação natural não florestal",
    "Water"                         = "Água",
    "Pasture"                       = "Pastagem",
    "Agriculture"                   = "Agricultura",
    "Silviculture"                  = "Silvicultura",
    "Oil palm"                      = "Cultura de palma",
    "Mosaic of uses"                = "Mosaico de usos",
    "Urban"                         = "Urbano",
    "Mining"                        = "Mineração",
    "Other anthropic"               = "Outras antrópicas",
    "Other"                         = "Outras"
  ),
  en = c(
    "Forest"                        = "Natural forest",
    "Non-forest natural formation"  = "Non-forest natural formation",
    "Water"                         = "Water",
    "Pasture"                       = "Pasture",
    "Agriculture"                   = "Agriculture",
    ...
    "Urban"                         = "Urban",
    "Mining"                        = "Mining",
    "Other anthropic"               = "Other anthropic",
    "Other"                         = "Other"
  )
)
mb_labels_for <- function(lang) MB_CLASS_I18N[[lang]]
...
## 1.4.1 Grouping helper (raw -> 8 final classes) ----
# ------------------------------------------------------------------------- - - -
mk_group_function <- function() {
  map_one <- function(x) {
    x <- as.character(x)
    dplyr::case_when(
      x == "Forest"                        ~ "Forest",
      x == "Non-forest natural formation"  ~ "Non-forest natural formation",
      x == "Water"                         ~ "Water",
      x == "Pasture"                       ~ "Pasture",
      x %in% c("Agriculture","Silviculture","Oil palm") ~ "Agriculture",
      x == "Mosaic of uses"                ~ "Pasture",
      x == "Urban"                         ~ "Urban",
      x == "Mining"                        ~ "Mining",
      x %in% c("Other anthropic non-vegetated","Other") ~ "Other anthropic",
      TRUE                                 ~ "Other anthropic"  # any unknowns
    )
  }
  function(v) vapply(v, map_one, character(1), USE.NAMES = FALSE)
}


# If legend is too wide, wrap into rows. Tweak this default if needed.
LEGEND_NROW <- 2L
...
build_alluvial_long_3 <- function(d01, d12, class_order) {
  ...
  # Outgoing proportions mid (T1→T2)
  prop_d12 <- d12_clean %>%
    group_by(mid) %>%
    summarise(transitions = list(tibble(dst = dst, area12 = area12)),
              total_mid_out = sum(area12, na.rm = TRUE), .groups = "drop") %>%
    unnest(transitions) %>%
    mutate(prop = if_else(total_mid_out > 0, area12 / total_mid_out, 0)) %>%
    select(mid, dst, prop) %>%
    filter(prop >= MIN_PROP_PER_MID)
...
build_alluvial_long_4 <- function(d01, d12, d23, class_order,
  ...
  p12 <- d12a %>% group_by(mid1) %>%
    mutate(total = sum(area12, na.rm = TRUE),
           prop12 = if_else(total > 0, area12/total, 0)) %>% ungroup() %>%
    select(mid1, mid2, prop12) %>% filter(prop12 >= min_prop_mid1)

  p23 <- d23a %>% group_by(mid2) %>%
    mutate(total = sum(area23, na.rm = TRUE),
           prop23 = if_else(total > 0, area23/total, 0)) %>% ungroup() %>%
    select(mid2, dst, prop23) %>% filter(prop23 >= min_prop_mid2)
...
plot_sankey_mb <- function(long_df, territory, lang = "pt") {
  ...
  # present classes in this dataset
  present_classes <- intersect(mb_order, levels(droplevels(long_df$class)))

  # sanity-check: every present class must have a color in the palette
  missing_cols <- setdiff(present_classes, names(mb_palette))
  if (length(missing_cols)) {
    stop("Palette missing colors for: ", paste(missing_cols, collapse = ", "))
  }
  ...
  ggplot(long_df,
    ...
    geom_flow(
      stat = "alluvium",
      lwd = 0.2,                    # thin outline improves definition
      color = scales::alpha("white", 0.20),
      alpha = FLOW_ALPHA,
      curve_type = "cubic",
      knot.pos = 0.30,              # slightly earlier bend (less wobble)
      lode.guidance = "frontback"   # ggalluvial tip to reduce crossings
    ) +
    geom_stratum(
      width = STRATUM_WIDTH,
    ...
    scale_fill_manual(
      values = mb_palette[present_classes],
      breaks = present_classes,
      labels = mb_labels_for(lang)[present_classes],
      guide  = guide_legend(
...
make_sankey_mb <- function(
  ...
  # APPLY CLASS GROUPING (to 8 final classes)
  if (AGGREGATE_CLASSES) {
    group_fn <- mk_group_function()
    df_clean <- df_clean %>%
      mutate(
        src = group_fn(src),
        dst = group_fn(dst)
  ...
  # Build long alluvial table
  if (n_stages == 4L) {
    long <- build_alluvial_long_4(
      d01, d12, d23, mb_order,
    ...
    long <- build_alluvial_long_3(d01, d12, mb_order) %>%
      mutate(
        stage = factor(stage, levels = c("src_T0","mid","dst_T2"),
