...
## 4) Checks & grooming ----
# -----------------------------------------------------------------------------
# Expected columns: year, precip_mm
stopifnot(all(c("year", "precip_mm") %in% names(df)))
df <- df %>% filter(!is.na(year), !is.na(precip_mm)) %>% arrange(year)

## 5) Plot ----
# -----------------------------------------------------------------------------
p <- ggplot(df, aes(x = factor(year), y = precip_mm)) +
  geom_boxplot(outlier.alpha = 0.25, width = 0.65) +
  labs(title = label("precip_box_title", .LANG = LANG),
       x = label("axis_year", .LANG = LANG),
       y = label("axis_precip_mm", .LANG = LANG)) +
