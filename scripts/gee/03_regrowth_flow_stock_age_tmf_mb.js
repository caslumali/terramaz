//============================================================================================
// TERRAMAZ — FOREST REGROWTH STOCK, FLOW & AGE CALCULATOR (TMF + MAPBIOMAS ATBD)
//============================================================================================
/**
 * Title: TerrAmaz — Forest Regrowth Stock, Flow & Age Calculator
 * Purpose: Calculate annual forest regrowth metrics following ATBD methodology
 * Author: Lucas Lima
 *
 * OVERVIEW:
 * This script calculates three types of metrics:
 * 
 ...
 *    - TMF: 1990-2024 | MapBiomas: 1987-2024
 * 
 ...
 *    - TMF: 1990-2024 | MapBiomas: 1987-2022 (requires t+2 validation)
 *
 * 3. AGE (Idade): Age distribution of secondary forest in 2024
 *    - Years since LAST regrowth event for each pixel
 *    - MapBiomas only (1-37 years, from events detected 1987-2022)
 *    - Only includes pixels still natural in 2024
 *
 * OUTPUT:
 * For each zone, 3 CSV files:
 * - {zone}_regrowth_stock_tmf_mb_1987_2024.csv → columns: area_id, year, tmf_px, tmf_area_ha, mb_px, mb_area_ha
 * - {zone}_regrowth_flow_tmf_mb_1987_2024.csv  → columns: area_id, year, tmf_px, tmf_area_ha, mb_px, mb_area_ha
 * - {zone}_age_mb_1987_2024.csv                → columns: age_years, area_ha
 *
 * IMPORTANT NOTES:
 * - MapBiomas regrowth detection limited to 2022 (needs t+2 ≤ 2024 per ATBD rules)
 * - Age calculation uses last regrowth event, not first (accounts for re-clearing)
 * - Minimum patch size: 0.5 ha (applied after trajectory analysis)
 */
...
var MB_YEAR_MAX = 2023;

// Regrowth events need t+2 → last event year is 2021
var MB_LAST_YEAR = MB_YEAR_MAX;                 // 2023
var MB_LAST_EVENT_YEAR = MB_LAST_YEAR - 2;      // 2021
...
function createMapBiomasTimeSeries() {
  var yearsList = ee.List.sequence(MB_YEAR_MIN - 2, MB_LAST_YEAR);
...
function createMBRegrowthEventsCollection() {
  ...
  var validEventYears = ee.List.sequence(MB_YEAR_MIN, MB_LAST_EVENT_YEAR);
...
//============================================================================================
// 6) FUNCTIONS TO CALCULTE ALL METRICS
//============================================================================================
/** Function to calcule stocl: Total secondary forest area in each year
 *    - All pixels that are regrowth in year t (regardless of when regeneration started)
 **/ 
...
/** Function to calculte the flow : New/recurrent regeneration events per year
 *    - Pixels that BECAME regrowth in year t (were NOT regrowth in t-1)
 *    - Allows recounting if pixel was cleared and regenerated again
 **/
// Function to calculate the flow of regeneration
...
/**
 * Function to calculate age distribution of secondary forest in 2023
 * Output: CSV with columns [area_id, age_years, area_ha]
 * Logic:
 *   - Last regrowth event year per pixel (ATBD rules)
 *   - Age = 2023 - last regrowth year
 *   - Keep only pixels that are still forest in 2023
 *   - Compute area (ha) per age class (1–37 years)
 */
function calculateAgeDistribution(feature) {
  feature = ee.Feature(feature);
  var zoneId = feature.get('zone');
  var geometry = feature.geometry().transform(MB_PROJ, 1);
  
  // Define the last valid year for MapBiomas Amazonia
  var LAST_YEAR = MB_LAST_YEAR;            // 2023
  var FIRST_EVENT_YEAR = MB_YEAR_MIN;      
  var AGE_MAX = LAST_YEAR - FIRST_EVENT_YEAR; 
  
  // 1) Last regrowth year per pixel (events up to 2021)
  var lastRegrowthYear = MB_REGROWTH_EVENTS.map(function(img) {
    var year = ee.Number(img.get('event_year'));
    return img.multiply(year).selfMask().rename('last_year').toInt16();
  }).max();

  // 2) Age = LAST_YEAR - lastRegrowthYear
  var age = ee.Image(LAST_YEAR).subtract(lastRegrowthYear);

  // 3) Forest mask in LAST_YEAR (use native classes, not aggregated)
  var naturalCodes = ee.List([3,4,5,6,11,12,29,13]); // forest-related MB classes
  var mbLast = MB_AMZ.select('classification_' + LAST_YEAR);
  var isForest = createMembershipMask(mbLast, naturalCodes);

  // 4) Valid pixels = had event + still forest in LAST_YEAR
  var validAge = age.updateMask(lastRegrowthYear.gt(0).and(isForest));

  // 5) Area per age class (1..AGE_MAX)
  var ageResults = ee.List.sequence(1, AGE_MAX).map(function(a) {
    a = ee.Number(a);
    var areaHa = calculateAreaHa(validAge.eq(a), geometry, MB_PROJ);
    return ee.Feature(null, { 'area_id': zoneId, 'age_years': a, 'area_ha': areaHa });
  });

  return ee.FeatureCollection(ageResults);
}
...
function exportResults() {
  ...
  ZONES.evaluate(function(featureCollection) {
    featureCollection.features.forEach(function(feature) {
      ...
      // AGE Export (MapBiomas only)
      var ageTable = calculateAgeDistribution(ee.Feature(feature));
      Export.table.toDrive({
        collection: ageTable,
        description: areaId + '_regrowth_age_mb_1987_2023',
        folder: areaId + '_Terramaz_metrics',
        fileNamePrefix: areaId + '_regrowth_age_mb_1987_2023',
        fileFormat: 'CSV'
      });
      print('  ✓ AGE export queued');
    });
