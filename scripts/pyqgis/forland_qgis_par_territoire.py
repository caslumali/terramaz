...
def build_territory_project(territory_key):
    """
    Build a single TerrAmaz project for one territory,
    including all indicators:
      - Forest Integrity (und/deg/reg)
      - Fire Vulnerability
      - Riskmap (Deforisk)
      - Boundary and non-forest mask

    The logic for copying datasources, applying styles and setting
    metadata is the same as in the individual project builders.
    """
    fi_info = FI_LAYERS[territory_key]
    fire_info = FIRE_LAYERS[territory_key]
    risk_info = RISKM_LAYERS[territory_key]
    ...
    print(f"Building full TerrAmaz project for {territory_label}...")

    # Output folders for this territory
    out_dir = os.path.join(BASE_OUTPUT_DIR, territory_key)
    rasters_dir = os.path.join(out_dir, "rasters")
    shapes_dir = os.path.join(out_dir, "shapes")
    os.makedirs(rasters_dir, exist_ok=True)
    os.makedirs(shapes_dir, exist_ok=True)
    ...
    # Project file
    project_path = os.path.join(out_dir, f"{territory_key}_TerrAmazIndicators.qgz")
    ...
    grp_boundary = root.addGroup("Boundary")
    grp_fi = root.addGroup("Forest Integrity")
    grp_fire = root.addGroup("Fire Vulnerability")
    grp_risk = root.addGroup("Riskmap")
    grp_masks = root.addGroup("Masks")
    ...
    apply_style(boundary_layer, COMMON_STYLES["boundary"])
    copy_style_for_datasource(COMMON_STYLES["boundary"], boundary_path)
    add_layer_to_group(proj, grp_boundary, boundary_layer)

    # Set initial project view centered on the boundary
    set_project_view_to_layer_extent(proj, boundary_layer)

    # --- Non-forest mask raster ---
    mask_master = get_layer_by_name(MASK_LAYERS[territory_key])
    mask_path = copy_datasource(mask_master, rasters_dir)
    mask_layer = QgsRasterLayer(mask_path, MASK_LAYERS[territory_key])
    apply_style(mask_layer, COMMON_STYLES["mask_non_forest"])
    ...
    # --- Forest Integrity rasters (und, deg, reg) ---
    md_fi = build_metadata_forest_integrity()
    ...
    for band_key in ["und", "deg", "reg"]:
        ...
        master_layer = get_layer_by_name(layer_name)
        raster_path = copy_datasource(master_layer, rasters_dir)
        fi_layer = QgsRasterLayer(raster_path, layer_name)
        apply_style(fi_layer, FI_STYLES[band_key])
        copy_style_for_datasource(FI_STYLES[band_key], raster_path)
        fi_layer.setMetadata(md_fi)
        add_layer_to_group(proj, grp_fi, fi_layer)

    # --- Fire Vulnerability raster ---
    fire_master = get_layer_by_name(fire_info["layer"])
    fire_raster_path = copy_datasource(fire_master, rasters_dir)
    fire_layer = QgsRasterLayer(fire_raster_path, fire_info["layer"])
    apply_style(fire_layer, fire_info["style"])
    ...
    # Save project
    proj.write()
    print(f"Saved full TerrAmaz project: {project_path}")
...
def run_all():
    """
    Main entry point: build one encapsulated project per territory,
    each containing all indicators (Forest Integrity, Fire Vulnerability,
    Riskmap), plus boundary and non-forest mask.
    """
    territories = ["cotriguacu", "paragominas", "guaviare", "madre_de_dios"]

    for t in territories:
        build_territory_project(t)

    print("All TerrAmaz territory projects generated successfully.")
